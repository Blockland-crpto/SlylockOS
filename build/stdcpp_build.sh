#!/bin/bash
sources=$(find ./libs/stdcpp/* -type f -name "*.c")
...
ta=$(echo ./stdcppbin/$(basename $(echo $objects | cut -d" " -f$i )))
...
cppsources=$(find ./libs/stdcpp/* -type f -name "*.cpp")
...
tap=$(echo ./stdcppbin/$(basename $(echo $cppobjects | cut -d" " -f$i )))
...
ar rcs lib/stdcpp.a stdcppbin/*.o
