#
sources=$(find ./libs/libdelegate/* -type f -name "*.c")
objects=$(echo ${sources//\.c/.o})
objb=''
char=' '
end=$(awk -F"${char}" '{print NF-1}' <<< "${objects}")
end=$((end+1))
mkdir libdelegatebin
for i in $(seq 1 $end); do 
ta=$(echo ./libdelegatebin/$(basename $(echo $objects | cut -d" " -f$i )))
...
cppsources=$(find ./libs/libdelegate/* -type f -name "*.cpp")
...
tap=$(echo ./libdelegatebin/$(basename $(echo $cppobjects | cut -d" " -f$i )))
...
ar rcs lib/libdelegate.a libdelegatebin/*.o
